...
# it can run the program there, in this case Main (Main.class)
...
# get to cron's table in editor mode with the command crontab -e
...
# file somewhere. other helpful commands are crontab -l to see what jobs you have scheduled
# and the command date will print what time your computer thinks it is
# so hopefully that made sense
# basically write a script that does to your folder where the program is (already compiled)
# then tell cron where that script is and when you want it run
